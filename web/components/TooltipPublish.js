...
class TooltipPublish extends React.Component {
  ...
  handlePublishClick(){
    ...
    const categories = R.map(({ text }) => {
      const trueCategory = R.find(({ name }) => name === text, this.props.categories);
  ...
  handleDeleteCategory(i){
    ...
    this.setState({ categories: filterIndex((e, idx) => idx !== i, categories) });
  }

  handleAddCategory(category){
    const { categories } = this.state;
    this.setState({ categories: categories.concat({ text: category, id: categories.length + 1 }) })
  }

  render(){
    return <div className="tooltip-publish-container p-md">
      <h3>Ready to publish?</h3>
      <div>Add or change categories so readers know what your story is about:</div>
      <ReactTags tags={this.state.categories}
      ...
      <div>Tip: add a high resolution image to your story to capture people’s interest</div>
      <div>
        <div>
          <i className="fa fa-twitter"/> Connect to share on Twitter
        </div>
        <div className="m-t-md">
          <i className="fa fa-facebook-official"/> Connect to share on Facebook
        </div>
      </div>
      <div>
        <div>
        ...
        <button onClick={() => this.handlePublishClick()} className="medium-btn btn--publish">Publish</button>
      </div>
    </div>
  }
}
